...
app.get("/user/following/", authenticateToken, async (request, response) => {
  ...
  WHERE follower.follower_user_id=${userDetails.user_id};`;
  const userFollowingDetails = await db.all(getUserFollowingQuery);
  response.send(userFollowingDetails);
});
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  const userFollowingDetails = await db.all(getUserFollowersQuery);
  response.send(userFollowingDetails);
});
